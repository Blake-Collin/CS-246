...
public class CreatureHandler {
  ...
  /** Sets the initial positions of the creatures to random locations within the world's grid. */
  ...
  private void wrapPoint(Point p) {
    ...
    // to the other side.
    if (p.x > _cols) p.x = 0;
    else if (p.x < 0) p.x = _cols;

    // If we go too far up or down, wrap around
    // to the other side.
    if (p.y > _rows) p.y = 0;
    else if (p.y < 0) p.y = _rows;
  }
  ...
  private Creature getTarget(Creature source, int xOffset, int yOffset) {
    ...
    for (Creature c : _creatures) {
      if (c == source || !c.isAlive()) continue;

      if (c.getLocation().x == newPoint.x && c.getLocation().y == newPoint.y) return c;
    }
  ...
  /**
   * Updates all of the living creatures in the game by making they each get a chance to act on
   * their implemented behaviors.
   */
  public void updateCreatures() {



  	//List for new creatures created during creature update
